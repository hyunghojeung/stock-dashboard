  ...
  return (<div>
    ...
    <div style={{ background:COLORS.card, border:`1px solid ${COLORS.cardBorder}`, borderRadius:12, padding:14, marginBottom:12, display:'flex', flexWrap:'wrap', gap:12, alignItems:'center' }}>
      <div style={{ display:'flex', gap:4 }}>
        ...
        {[{v:'all',l:'전체',c:COLORS.accent,t:'모든 급상승 종목 표시'},{v:'high',l:'🔴 세력의심',c:COLORS.red,t:'거래량 폭증, 급등 후 급락 등 세력 개입 가능성이 높은 종목'},{v:'medium',l:'🟡 주의이상',c:COLORS.yellow,t:'세력 의심 + 주의 필요 등급 종목 (중간 이상 위험)'},{v:'entry',l:'⚡ 진입시그널',c:'#00BCD4',t:'OBV+VCP+DTW 합산 점수가 임계값을 초과한 매수 추천 종목'},{v:'obv',l:'OBV',c:'#4FC3F7',t:'OBV 다이버전스: 주가 횡보 중 거래량 축적 감지 (세력 매집 징후)'},{v:'vcp',l:'VCP',c:'#AB47BC',t:'볼린저 스퀴즈: 변동성 수축 후 급등 가능성 (눌림목 패턴)'},{v:'dtw',l:'DTW',c:'#FF7043',t:'부분 DTW 매칭: 과거 급등 패턴 초기 구간과 유사한 종목'}].map(f => (<button key={f.v} onClick={() => setScanFilterLevel(f.v)} title={f.t} style={{ padding:'5px 12px', fontSize:11, borderRadius:6, cursor:'pointer', border:`1px solid ${scanFilterLevel===f.v?f.c:COLORS.cardBorder}`, background:scanFilterLevel===f.v?`${f.c}20`:'transparent', color:scanFilterLevel===f.v?f.c:COLORS.textDim }}>{f.l}</button>))}
      </div>
      <div style={{ display:'flex', gap:4 }}>
